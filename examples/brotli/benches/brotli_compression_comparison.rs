...
use ef_brotli_lib::brotli::Brotli;
use ef_brotli_lib::{with_mpkrt_lib, test_brotli, test_brotli_unsafe};
    ...
    encapfn::branding::new(|brand| {
        with_mpkrt_lib(brand, |lib, mut alloc, mut access| {
            ...
            for size in [8, 64, 128, 256, 512, 1024] {
                ...
                group.bench_with_input(
                    BenchmarkId::new("ef_mpk", tput_bytes),
                    &tput_bytes,
                    |b, _| {
                        for _ in 0..STACK_RANDOMIZE_ITERS {
                            let stack_bytes: usize = (&mut prng)
                            ...
                            push_stack_bytes(stack_bytes, || {
                                lib.rt()
                                    .allocate_stacked_mut(
                                        ...
                                        |_, alloc| {
                                            // println!("Pushed {} bytes onto the stack...", stack_bytes);
                                            b.iter(|| black_box(test_brotli(&lib, alloc, &mut access, black_box(size))));
                                        },
                                    )
                                    .unwrap();
                            });
