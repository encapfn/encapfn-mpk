...
use ef_libpng_lib::libpng_bindings::LibPng;
use ef_libpng_lib::{ef_mpk, unsafe_ffi};
...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    encapfn::branding::new(|brand| {
        ef_mpk::with_mpkrt_lib(brand, |lib, mut alloc, mut access| {
            let mut group = c.benchmark_group("libpng_decode");

            // Allocate a buffer in the EF MPK domain:
            let ef_mpk_dst_buffer: *mut u8 = lib
                                        ...
                                        |_, alloc| {
                                            ...
                                            b.iter(|| {
                                                let (png_ptr, info_ptr) =
                                                    ef_mpk::libpng_init(&lib, alloc, &mut access);
                                                ...
                                                ef_mpk::decode_png(
                                                    ...
                                                    info_ptr,
                                                    png_image,
                                                    Some((ef_mpk_dst_buffer, max_buffer_size)),
                                                    |_, _, _, _, _| (),
                                                );

                                                ef_mpk::libpng_destroy(
                                                    &lib,
                                                    alloc,
                                                    &mut access,
                                                    png_ptr,
