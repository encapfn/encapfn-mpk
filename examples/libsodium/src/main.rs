...
use ef_libsodium_lib::{
    calc_hash,
    // calc_hash_unsafe, calc_hash_validate, libsodium_hash_ef, libsodium_hash_unsafe,
    // libsodium_public, libsodium_public_unsafe, libsodium_public_validate, with_mockrt_lib,
    with_mpkrt_lib,
};
...
// ) {
//     libsodium_hash_ef(lib, alloc, access);
...
fn main() {
    ...
    encapfn::branding::new(|brand| {
        with_mpkrt_lib(brand, |lib, mut alloc, mut access| {
            ...
            // b.iter(|| libsodium_hash_ef(&lib, &mut alloc, &mut access, black_box(&to_hash)));
