...
fn main() {
    ...
    println!("cargo:rerun-if-changed=./c_src/efdemo.h");
    println!("cargo:rerun-if-changed=./c_src/efdemo.c");
    ...
    bindings
        .write_to_file(out_path.join("libefdemo_bindings.rs"))
        .expect("Couldn't write bindings!");

    // Build the libefdemo as a shared library.
    //
    ...
    check_output_res(
        std::process::Command::new(&cc)
            .args([
                ...
                OsStr::new("c_src/efdemo.c"),
                //OsStr::new("c_src/testmalloc.c"),
                OsStr::new("-o"),
                out_path.join("libefdemo.so").as_os_str(),
            ])
            .output(),
    ...
    println!("cargo:rustc-link-lib=efdemo");
