...
| Omniglot     | ???????????????? | ${BENCH_RESULTS["runtime_setup/ef_mpk"]} | ???????????????? | ${BENCH_RESULTS["ubench_invoke/ef_mpk/0"]} |
...
BROTLI_ISOLATION="${BENCH_RESULTS["brotli-unchecked/brotli_compress_decompress/ef_mpk/1024"]}"
BROTLI_OMNIGLOT="${BENCH_RESULTS["brotli-checked/brotli_compress_decompress/ef_mpk/1024"]}"
BROTLI_ISOLATION_OVERHEAD="$(\
	compute_overhead \
		"${BENCH_RESULTS_EST["brotli-unchecked/brotli_compress_decompress/ef_mpk/1024"]}" \
		"${BENCH_RESULTS_EST["brotli-unchecked/brotli_compress_decompress/unsafe/1024"]}")"
BROTLI_OMNIGLOT_OVERHEAD="$(\
	compute_overhead \
		"${BENCH_RESULTS_EST["brotli-checked/brotli_compress_decompress/ef_mpk/1024"]}" \
		"${BENCH_RESULTS_EST["brotli-unchecked/brotli_compress_decompress/ef_mpk/1024"]}")"

SODIUM_UNSAFE="${BENCH_RESULTS["sodium-unchecked/libsodium_hash/unsafe/32768"]}"
SODIUM_ISOLATION="${BENCH_RESULTS["sodium-unchecked/libsodium_hash/ef_mpk/32768"]}"
SODIUM_OMNIGLOT="${BENCH_RESULTS["sodium-checked/libsodium_hash/ef_mpk/32768"]}"
SODIUM_ISOLATION_OVERHEAD="$(\
	compute_overhead \
		"${BENCH_RESULTS_EST["sodium-unchecked/libsodium_hash/ef_mpk/32768"]}" \
		"${BENCH_RESULTS_EST["sodium-unchecked/libsodium_hash/unsafe/32768"]}")"
SODIUM_OMNIGLOT_OVERHEAD="$(\
	compute_overhead \
		"${BENCH_RESULTS_EST["sodium-checked/libsodium_hash/ef_mpk/32768"]}" \
		"${BENCH_RESULTS_EST["sodium-unchecked/libsodium_hash/ef_mpk/32768"]}")"

LIBPNG_UNSAFE="${BENCH_RESULTS["libpng-unchecked/libpng_decode/unsafe/big_building_001.0p.png"]}"
LIBPNG_ISOLATION="${BENCH_RESULTS["libpng-unchecked/libpng_decode/ef_mpk/big_building_001.0p.png"]}"
LIBPNG_OMNIGLOT="${BENCH_RESULTS["libpng-checked/libpng_decode/ef_mpk/big_building_001.0p.png"]}"
LIBPNG_ISOLATION_OVERHEAD="$(\
	compute_overhead \
		"${BENCH_RESULTS_EST["libpng-unchecked/libpng_decode/ef_mpk/big_building_001.0p.png"]}" \
		"${BENCH_RESULTS_EST["libpng-unchecked/libpng_decode/unsafe/big_building_001.0p.png"]}")"
LIBPNG_OMNIGLOT_OVERHEAD="$(\
	compute_overhead \
		"${BENCH_RESULTS_EST["libpng-checked/libpng_decode/ef_mpk/big_building_001.0p.png"]}" \
		"${BENCH_RESULTS_EST["libpng-unchecked/libpng_decode/ef_mpk/big_building_001.0p.png"]}")"
